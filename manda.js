...
class yagateou{
  ...
  make(){
    var temp=`
<section id="${this.id}" class="frame">
  <div class="wrap">
    <div class="body"
...
`
    ...
    this.ed = this.wrap.children[0];
    this.img = this.wrap.children[1];
  }
  async checkTitle(){
...
export class yagateouSite{
  ...
  async makeNav(){
    ...
    .map(d=>`<a href="#${d.parentElement.parentElement.id}">${d.dataset.title}</a>`)
    .join('\n')
